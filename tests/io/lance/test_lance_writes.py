...
@pytest.fixture(scope="function")
def minio_io_config():
    """Fixture for minio IO config. Skips test if not available."""
    pytest.skip("MinIO integration test - requires MinIO configuration")


@contextlib.contextmanager
def minio_create_bucket(minio_io_config, bucket_name):
    """Context manager for creating MinIO bucket."""
    yield
...
@pytest.mark.xfail(reason="Daft LanceDataSink error handling differs from expected")
